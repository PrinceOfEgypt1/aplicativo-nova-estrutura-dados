...
import { metodosVetor } from "./metodosVetor";
    ...
    <div className="space-y-4">
      ...
      <div className="space-y-2">
        ...
        <ul className="space-y-2">
          {Object.entries(metodosVetor).map(([chave, metodo]) => (
            <li key={chave} className="border-l-4 border-blue-500 pl-4">
              <h4 className="font-mono font-bold">{metodo.nome}</h4>
              <p className="text-sm text-gray-600 dark:text-gray-400">
                {metodo.descricao}
              </p>
              {metodo.parametros.length > 0 && (
                <p className="text-sm font-mono text-gray-500 dark:text-gray-400">
                  Parametros: {metodo.parametros.join(", ")}
                </p>
              )}
            </li>
          ))}
